...
import {
  runDir, manifestPath, journalPath, listingsPath, enrichedPath, reportPath,
  routeTrialRequestPath, routeTrialResultPath, valuationReviewPath,
} from './runpaths.ts';
...
test('all run paths resolve under state/runs/<profile>/<label>/ for a single-day label', () => {
  ...
  assert.equal(routeTrialRequestPath(P, L), 'state/runs/owner-occupied/2026-06-26/route-trial-request.json');
  assert.equal(routeTrialResultPath(P, L), 'state/runs/owner-occupied/2026-06-26/route-trial.json');
...
test('all run paths resolve under state/runs/<profile>/<label>/ for a range label', () => {
  ...
  assert.equal(routeTrialRequestPath(P, L), 'state/runs/investment/2026-06-20_2026-06-25/route-trial-request.json');
  assert.equal(routeTrialResultPath(P, L), 'state/runs/investment/2026-06-20_2026-06-25/route-trial.json');
