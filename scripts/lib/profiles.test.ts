...
test('availableProfileIds discovers on-disk profile folders, sorted', () => {
  ...
  assert.ok(ids.includes('example-investment'));
  assert.ok(ids.includes('example-owner-occupied'));
  assert.deepEqual(ids, [...ids].sort());
});

test('loadProfile returns id (=folder), displayName, and fetch map', () => {
  const p = loadProfile('example-investment');
  assert.equal(p.id, 'example-investment');
  assert.equal(p.displayName, 'iBigFun 台北投資房源監測（範例）');
  assert.deepEqual(p.fetch.price_segment, { max: 3000 });
  assert.equal(p.fetch.city, '1');
});

test('loadProfile reads owner-occupied fetch arrays + ranges', () => {
  const p = loadProfile('example-owner-occupied');
  assert.deepEqual(p.fetch.town, ['1', '4']);
...
test('resolveProfileFromArgs requires --profile and accepts both forms', () => {
  ...
  assert.equal(resolveProfileFromArgs(['--profile', 'example-investment']).id, 'example-investment');
  assert.equal(resolveProfileFromArgs(['--profile=example-owner-occupied']).id, 'example-owner-occupied');
});

test('profileFlags reproduces the selected profile flag', () => {
  assert.equal(profileFlags({ id: 'example-investment' } as Profile), '--profile example-investment');
});
